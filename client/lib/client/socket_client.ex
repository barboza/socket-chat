defmodule Client.SocketClient do
  @moduledoc false
  require Logger
  ...
  def handle_connected(transport, state) do
    ...
    GenSocketClient.join(transport, "lobby:1")
    {:ok, state}
  end
  ...
  def handle_joined(topic, _payload, _transport, state) do
    ...
    if state.first_join do
      ...
      :timer.send_interval(:timer.seconds(5), self(), :broadcast)
  ...
  def handle_info(:ping_server, transport, state) do
    ...
    GenSocketClient.push(transport, "lobby:1", "ping", %{ping_ref: state.ping_ref})
    {:ok, %{state | ping_ref: state.ping_ref + 1}}
  end

  def handle_info(:broadcast, transport, state) do
    Logger.info("sending broadcast to ##{state.ping_ref}")
    GenSocketClient.push(transport, "lobby:1", "shout", %{ping_ref: state.ping_ref})
    {:ok, %{state | ping_ref: state.ping_ref + 1}}
  end
